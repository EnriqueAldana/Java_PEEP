 ...
 * 
 * TATA Consulting Services - Jesus Enrique Aldana Sanchez.
 *******************************************************************************/
...
public class CaptureScreenToFile
{
  ...
  private  int fps=24;
  private  double segundosDeVideo=60;
  public int getFps() {
	return fps;
}
...
public double getSegundosDeVideo() {
	return segundosDeVideo;
}
public void setSegundosDeVideo(double segundosDeVideo) {
	this.segundosDeVideo = segundosDeVideo;
}
// Quality Image
  private int bitRate=250000;
  
  private  boolean startUpId=false;
 
  
 
  public  boolean isStartUpId() {
		return startUpId;
	}
...
public void CaptureScreenToFile(){
  ...
  public  CaptureScreenToFile(String outFile)throws RuntimeException
  {
    try
    {
      robot = new Robot();
    ...
    if (retval < 0) {
    	logApp.warning("could not open output file" + outFile);
...
//ICodec codec = ICodec.findEncodingCodec(ICodec.ID.CODEC_ID_H264);
    ...
    if (codec == null){
    	logApp.warning("could not guess a codec" + outFile);
...
// Change bitRate value for best quality video
    outStreamCoder.setBitRate(bitRate);
    outStreamCoder.setBitRateTolerance(9000);

    int width = toolkit.getScreenSize().width;
    int height = toolkit.getScreenSize().height;
    //ICodec.Type.swigToEnum(XugglerJNI.IStreamCoder_getCodecType(swigCPtr, this));
    ...
    outStreamCoder.setFrameRate(getFrameRate());
    outStreamCoder.setTimeBase(IRational.make(getFrameRate().getDenominator(),
        getFrameRate().getNumerator()));

    retval = outStreamCoder.open(null, null);
    if (retval < 0){
    	logApp.warning("could not open input decoder" + outFile);
      throw new RuntimeException("could not open input decoder");
    }
    retval = outContainer.writeHeader();
    if (retval < 0){
    	logApp.warning("could not write file header" + outFile);
